#!/usr/bin/env bash
# dopamine v1.6.0 — spinning roulette wheel + dice tumble + RPS shoot, plus the v1.5
# JS-driven dealing/spinning. Run from inside the dopamine-repo folder:  bash apply_ui.sh
set -euo pipefail
cd "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
[ -f pyproject.toml ] && [ -d dopamine ] || { echo "ERROR: run from the dopamine-repo folder."; exit 1; }
echo ">> writing dopamine/render.py (spinning wheel + tumble + shoot)"
...
S_EOF
sed -i.bak -E 's/version = "1\.[0-9]+\.[0-9]+"/version = "1.6.0"/' pyproject.toml && rm -f pyproject.toml.bak
echo ""
echo "DONE. Now run:"
echo "   git add -A && git commit -m \"v1.6.0: spinning roulette wheel, dice tumble, rps shoot\" && git push"
